...
DegDetector(matrix_width, matrix_depth), slot_num(slotNum), exp_deg(exp_deg), ignored_bits(ignored_bits), reserved_bits(reserved_bits), BITVECBITS(reserved_bits), UPDATEBITS(update_bits), ADDRBITS(addr_bits)
{
    ...
    this->out_slots = (slot *) malloc(sizeof(slot) * slotNum * SLOTROOM);		    // 64-byte alignment of the requested space
    this->in_slots = (slot *) malloc(sizeof(slot) * slotNum * SLOTROOM);		    // 64-byte alignment of the requested space
    //cout<<"66";
    ...
    //cout<<"sizeof(slot) "<<sizeof(slot)<<endl;
    //cout<<"slotNum"<<slotNum<<endl;
    //cout<<"this->out_slots "<<this->out_slots<<endl;
    //cout<<"this->in_slots "<<this->in_slots<<endl;
