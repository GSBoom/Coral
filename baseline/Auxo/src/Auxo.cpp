...
bool Auxo::insert(string s1, string s2,int weight,double& insertMatrix)// s1 is the ID of the source node, s2 is the ID of the destination node, weight is the edge weight.
{
        ...
        while(mainBasketNow){
            ...
            for(int i=0;i<p;i++)
            {
                ...
                if (value->weight[pos]== 0)//如果出现这个可以说明这个边没有插入过了
                {
                    if(emptyNum==-1){
                        emptyNum=i;
        ...
        if(!inserted){
            ...
            MatrixAppend(tempMainBasket,mainLevelNow);//对这个节点进行扩展
            int srcFigSize=f-(mainLevelNow>>1),dstFigSize=f-(mainLevelNow>>1)-(mainLevelNow&0x1);
...
bool Auxo::inserttest(string s1, string s2,int weight,double& insertMatrix,unsigned int& number,double& migrationtime)// s1 is the ID of the source node, s2 is the ID of the destination node, weight is the edge weight.
{
        timeval t_start, t_end,ts,te;
        gettimeofday( &t_start, NULL);
		ul hash1 = mpq->MPQHash(s1.c_str(), Type);
		ul hash2 = mpq->MPQHash(s2.c_str(), Type);
		unsigned int g1 = hash1 & fvalue[f];
...
//            unsigned int srcFig=(g1>>(f-srcFigSize)),dstFig=(g2>>(f-dstFigSize));
            ...
            number++;
            migrationtime+=(te.tv_sec-ts.tv_sec)*1000000.0 +(te.tv_usec-ts.tv_usec);
        }
		delete [] tmp1;
		delete [] tmp2;
		delete [] keys;
...
/*
void Auxo::distanceknode(string s,int k){
    vector<string> increase,tem,res;
    nodeSuccessorQuery(s,increase);
    res.insert(res.end(),increase.begin(),increase.end());

    for(int m=0;m<k-1;m++){
        for(string s:increase){
            nodeSuccessorQuery(s,tem);
...
void Auxo::TopKheavyhitter(vector<string> &lotsnode){
    string maxid;
    int maxnu=0;
    double time;
    for(string s:lotsnode){
        int res=nodeValueQuery(s,0,time);
