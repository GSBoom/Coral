// Cache-friendliness test for GSS_Chain — node out-flow query
// Usage: ./test_gss_cache -dataset <path> -width <w> -maxedges <n> -query <file> -output <file>
...
int main(int argc, char* argv[]) {
    ...
    string output    = "cache_gss_result.txt";
    ...
    GSS_Chain gss(width, cols, candiNum, fpl);
    ...
    while (!ifs.eof() && (maxedges == 0 || loaded < maxedges)) {
        ifs >> s >> d >> w >> t;
        if (ifs.fail()) break;
        gss.insert(to_string(s), to_string(d), 1, insertTime);
        loaded++;
        if (loaded % 500000 == 0) cerr << "GSS pre-loaded " << loaded << " edges" << endl;
    }
    ifs.close();
    cerr << "INSERT_DONE" << endl;
    string gssReadyFile = output + ".ready";
    { ofstream rf(gssReadyFile); rf << getpid() << endl; rf.close(); }
    sleep(5);
    ...
    for (int i = 0; i < nQueries; i++) {
        int w = gss.nodeValueQuery(queries[i], 0, queryTime);  // type=0 = out-flow
        totalWeight += w;
    }

    // === Phase 4: Output ===
    ofstream ofs(output);
    ofs << "structure: GSS_Chain" << endl;
