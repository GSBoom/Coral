...
int insert(T& scube, string filename, int type, int data_interval) {		// int data_interval - granularity = 10000
	...
	if(total == 0) cout << "ERROR--QueryFunction--154" <<endl;
	cout<<"file: "<<filename<<" "<<total<<" lines"<<endl;
	/*
...
*/
	...
	while (!ifs.eof()) {
		...
		allinserttime+=(t2.tv_sec-t1.tv_sec)*1000000+t2.tv_usec-t1.tv_usec;
		datanum++;
		if(datanum%1000000==0){
			cout<<"scube has inserted "<<datanum<<" datas, use "<<allinserttime<<" useconds"<<endl;
		}
		/*
#if defined(DEBUG)
...
template <class T>
uint32_t edgeFrequenceQueryTest(T& scube, QueryData* qdata, int querynum, int query_times, string outputpath) {
	...
	for (int m = 0; m < query_times; m++) {
		...
		for (int n = 0; n < querynum; n++) {
			gettimeofday( &tp1, NULL);
			uint32_t res = scube.edgeWeightQuery(to_string(qdata[n].a), to_string(qdata[n].b));
			gettimeofday( &tp2, NULL);
			double delta_t = (tp2.tv_sec - tp1.tv_sec) * 1000000 +  (tp2.tv_usec - tp1.tv_usec);
			sumTime_perquery += delta_t;
			resultFile<<qdata[n].a<<" "<<qdata[n].b<<" "<<res<<endl;
		}
		sumTime += (sumTime_perquery / (double)querynum);
	}

	cout<<"this time has query "<<querynum<<" datas "<<",total time is "<<sumTime_perquery<<" useconds"<<endl;
	cout<<"average time is "<<sumTime_perquery / (double)querynum<<" useonds"<<endl;
	resultFile.close();
...
template <class T>
uint32_t edgeFrequenceQueryTest(T& scube, QueryData* qdata, int querynum, int query_times, string outputpath,int times) {
...
template <class T>
uint32_t nodeFrequenceQueryTest(T& scube,QueryData* qdata, int querynum, int query_times, string outputpath, int flag) {
	...
	if(flag==1){
		...
		for (int m = 0; m < query_times; m++) {
			...
			for (int n = 0; n < querynum; n++) {			
				gettimeofday( &tp1, NULL);
				int32_t res1 = scube.nodeWeightQuery(to_string(qdata[n].a), 1);
				int32_t res2 = scube.nodeWeightQuery(to_string(qdata[n].b), 1);
				gettimeofday( &tp2, NULL);
				double delta_t = (tp2.tv_sec - tp1.tv_sec) * 1000000 +  (tp2.tv_usec - tp1.tv_usec);
				sumTime_perquery += delta_t;	
				resultFile<<qdata[n].a<<" "<<res1<<" "<<qdata[n].b<<" "<<res2<<endl;
			}
			sumTime += (sumTime_perquery / (double)querynum);
		}
		cout<<"this time has query "<<querynum<<" datas "<<",total time is "<<sumTime_perquery<<" useconds"<<endl;
	...
	if(flag==0){
		...
		for (int m = 0; m < query_times; m++) {
			...
			for (int n = 0; n < querynum; n++) {			
				gettimeofday( &tp1, NULL);
				int32_t res1 = scube.nodeWeightQuery(to_string(qdata[n].a), 0);
				int32_t res2 = scube.nodeWeightQuery(to_string(qdata[n].b), 0);
				gettimeofday( &tp2, NULL);
				double delta_t = (tp2.tv_sec - tp1.tv_sec) * 1000000 +  (tp2.tv_usec - tp1.tv_usec);
				sumTime_perquery += delta_t;	
				resultFile<<qdata[n].a<<" "<<res1<<" "<<qdata[n].b<<" "<<res2<<endl;
		...
		cout<<"this time has query "<<querynum<<" datas "<<",total time is "<<sumTime_perquery<<" useconds"<<endl;
	    cout<<"average time is "<<sumTime_perquery / (2*(double)querynum)<<" useonds"<<endl;
	    resultFile.close();
		return 0;
	}
	return 0;
...
template <class T>
uint32_t nodeFrequenceQueryTest(T& scube,QueryData* qdata, int querynum, int query_times, string outputpath, int flag,int times) {
...
template <class T>
uint32_t reachabilityQueryTest(T& scube, QueryData* qdata, int querynum, int query_times, string outputpath) {
	...
	for (int m = 0; m < query_times; m++) {
		...
		for (int n = 0; n < querynum; n++) {
			gettimeofday( &tp1, NULL);
			bool res = scube.reachabilityQuery(to_string(qdata[n].a), to_string(qdata[n].b));
			gettimeofday( &tp2, NULL);
			double delta_t = (tp2.tv_sec - tp1.tv_sec) * 1000000 +  (tp2.tv_usec - tp1.tv_usec);
			sumTime_perquery += delta_t;
			resultFile<<qdata[n].a<<" "<<qdata[n].b<<" "<<res<<endl;
...
template <class T>
uint32_t reachabilityQueryTest(T& scube, QueryData* qdata, int querynum, int query_times, string outputpath,int times) {
